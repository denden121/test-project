...
|------------|--------|----------------|
| **Реалтайм на странице владельца** | ✅ | Страница управления подписана на WebSocket по `slug`; при резервации/вкладе владелец получает обновление и перезапрашивает список — видит изменения без перезагрузки. |
| **Восстановление пароля** | ✅ | `POST /auth/forgot-password` (email) и `POST /auth/reset-password` (token, new_password). При заданном RESEND_API_KEY письмо со ссылкой отправляется через Resend. |
