...
| **Реалтайм на странице владельца** | ✅ | Страница управления подписана на WebSocket по `slug`; при резервации/вкладе владелец по событию перезапрашивает список и видит изменения без перезагрузки. |
| **Восстановление пароля** | ✅ | Бэкенд: `POST /auth/forgot-password` (email), `POST /auth/reset-password` (token, new_password); при заданном RESEND_API_KEY письмо со ссылкой через Resend. Фронт: страницы «Забыли пароль» и «Новый пароль» (`/forgot-password`, `/reset-password?token=...`), authApi `forgotPassword` и `resetPassword`. |

### 7.2. Плюс по PRD — реализовано

| Требование | Статус | Реализация |
|------------|--------|------------|
| **Автозаполнение по URL товара** (п. 3.10) | ✅ | Эндпоинт `POST /api/wishlists/fetch-product` (url); парсинг Open Graph и schema.org. В форме товара (управление списком): кнопка «Заполнить по ссылке», подстановка title, image_url, price. |

### 7.3. Не реализовано

На момент проверки все требования разд. 1–5 (обязательные и плюс) закрыты. Открытых пробелов нет.

### 7.4. Сводка реализованного

- Вишлисты: создание (с поводом, датой, валютой), редактирование, удаление; публичная ссылка по `slug`; управление по `creator_secret`; привязка к пользователю и «мои вишлисты» по токену (`GET /wishlists/mine`).
- Товары: название, ссылка, цена, картинка, минимальный вклад; добавление/редактирование/удаление владельцем; автозаполнение по URL (fetch-product).
...
- WebSocket: реалтайм на публичной странице и на странице управления (резервации и вклады).
- Авторизация: email + пароль, Google OAuth, восстановление пароля (forgot/reset); хранение пароля в виде хеша (bcrypt).
...
**Итог:** Все обязательные и плюс-требования из этого документа реализованы.
