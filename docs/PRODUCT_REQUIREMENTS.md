...
| **Минимальный вклад** (опционально) | Нет | Нет полей `min_contribution` на уровне вишлиста или товара и проверки при внесении вклада. |
| **Нативный шаринг** | Только копирование | Есть копирование ссылки в буфер; нет вызова `navigator.share()` (Web Share API) для «Поделиться» в мессенджерах/соцсетях. |
